...
CLASS game IMPLEMENTATION.
  METHOD constructor.
    DATA(size) = 4.
    cl_demo_input=>add_field( EXPORTING text = 'Размер' CHANGING field = size ).
    cl_demo_input=>add_field( EXPORTING text = 'Цель' CHANGING field = target ).
